...
<%
remain=1500 '시간정의 sec:  3600=1hour
...
<form name="form1" method="post" action="testa_chk.asp" target="_top">
<input type="hidden" name="chkpage" value="5">
  ...
  document.form1.submit();
	...
	sub window_onload
	defaultstatus = "인성 검사시간이 초과되었습니다."
	end sub
	</script>
...
<%else%>
	...
	<script language=vbscript>
	sub window_onload
	defaultstatus = "인성검사 남은시간 : 1분 미만입니다."
	end sub
	</script>
	<%else%>
	<script language=vbscript>
	sub window_onload
	defaultstatus = "인성검사 남은시간 : <%=remaintime%>분 "
	end sub
	</script>
	<%end if%>
<%end if%>